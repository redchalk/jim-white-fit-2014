...
<script type="text/javascript">
	...
	jQuery(document).on( 'backupbuddy_archiveURL', function(e, json) {
		jQuery( '#pb_backupbuddy_archive_url' ).attr( 'href', '<?php echo pb_backupbuddy::ajax_url( 'download_archive' ) . '&backupbuddy_backup='; ?>' + json.data );
		jQuery( '#pb_backupbuddy_archive_send' ).attr( 'rel', json.data );
	...
	function backupbuddy_log( messages_output ) {
		/*
		if ( 0 === keep_polling ) {
			console.log( 'Undisplayed log after halt: ' + messages_output );
			return;
		}
		*/
			...
			success: function( data ) {
				...
				for( var i = 0; i < data.length; i++ ) {
					...
					try {
						...
						json.date = json.date.getHours() + ':' + json.date.getMinutes() + ':' + seconds;
						
						triggerEvent = 'backupbuddy_' + json.event;
						
						// Log non-text events.
						if ( ( 'details' !== json.event ) && ( 'message' !== json.event ) && ( 'details' !== json.event ) ) {
							console.log( 'Non-text event `' + triggerEvent + '`.' );
						}
						
						jQuery(document).trigger( triggerEvent, [json] );
						continue;
					} catch(e) {
	...
	function backupbuddyError( message ) {
		...
		troubleURL = '';
		error_number_begin = json.data.toLowerCase().indexOf( 'error #' );
		if ( error_number_begin >= 0 ) {
			error_number_begin += 7; // Shift over index to after 'error #'.
			error_number_end = json.data.toLowerCase().indexOf( ':', error_number_begin );
			...
			error_number = json.data.slice( error_number_begin, error_number_end );
...
<div style="width: 793px;">
	...
	*/
	...
	echo '<textarea wrap="off" id="backupbuddy_messages" style="width: 793px; white-space: nowrap;">Time	Elapsed	Memory		Message</textarea>';
	pb_backupbuddy::$ui->end_tab();
